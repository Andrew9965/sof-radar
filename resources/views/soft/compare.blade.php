        ...
        <div class="container">
            <div class="section__inner">
                <div class="section__title">
                    <h1>{{$product->title}} Comparisons</h1>
                </div>
                        ...
                        <aside class="col-lg-3 aside">
                            ...
                            <div class="filter js-filter-box alternative-filter">
                                <div class="filter__box">
                                    <form action="{{route('product.compare', ['product' => $product->slug])}}" id="filter-form">
                                            ...
                                            @if($category->filter && count(array_diff($category->filter, [0])))
                                                <div class="filter__header">
                                                    ...
                                                    @if(count(request()->all()))
                                                        <div class="_action"><button onclick="$(this).parents('form').find('[checked]').removeAttr('checked'); location='{{route('product.compare', ['product' => $product->slug])}}';" class="_btn" type="reset"><i></i>Clear</button></div>
                                                    @endif
                                                </div>
                                ...
                                <div class="review-item__inner">
                                    ...
                                    <div class="review-item__info">
                                        ...
                                        <div class="_action">
                                            ...
                                            @endif
                                            @php
                                                $c = $compares->where('product_left_id', $prod->id);
                                                if(!count($c)) $c = $compares->where('product_right_id', $prod->id);
                                                $c = array_first($c);
                                            @endphp
                                            @if($c) <a href="{{route('compare', ['compare' => $c->slug])}}" class="_btn btn btn-purple">Go to compare</a> @else
                                            <a href="{{route('compare.new', ['left' => $product->id, 'right' => $prod->id])}}" class="_btn btn btn-purple">Go to compare</a> @endif
                                        </div>
                                    </div>
