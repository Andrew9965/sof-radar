<div class="filter js-filter-box">
    <div class="filter__box">
        ...
        <form action="{{route('relate', ['related_links' => $category->slug])}}">
        @else
        <form action="{{route('category', ['category' => $category->slug])}}">
        @endif
            ...
            @if($category->filter && count(array_diff($category->filter, [0])) && count(request()->all()))
                @if(request()->related_links)
                    <div class="filter__clear"><button onclick="$(this).parents('form').find('[checked]').removeAttr('checked'); location='{{route('relate', ['related_links' => $category->slug])}}';" class="_btn js-filter-reset" type="reset"><i></i>Clear</button></div>
                @else
                    <div class="filter__clear"><button onclick="$(this).parents('form').find('[checked]').removeAttr('checked'); location='{{route('category', ['category' => $category->slug])}}';" class="_btn js-filter-reset" type="reset"><i></i>Clear</button></div>
                @endif
            @endif
        </form>
    </div>
